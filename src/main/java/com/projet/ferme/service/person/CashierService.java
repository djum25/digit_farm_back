...
public class CashierService {
	...
	public Map<String, Object> addCashier(Cashier cashier) {
		...
		if (exit && roleCashier) {
			...
			map.put("success", false);
			map.put("message", "L'enregistrement a échoué, ce caissier est deja dans cette boutique ou sont "
	...
	public Map<String, Object> activeCashier(Long id) {
		...
		if (cashierOptional.isPresent()) {
			...
			if (savedCashier == null)
				...
				return response(true, "Mise a jour avec succé");
		}else
			return response(false, "Cette utilisateur n'exite plus");
	}
		...
		if (cashierOptional.isEmpty()) {
			...
			if(access == cashierOptional.get().getAccess()) {
				if(cashierOptional.get().isStatus()) {
					map=open(cashierOptional.get(), false, "Votre caisse est fermé", shop, cash);
				}else {
					if(isOpen(shop.getId())) {
						map=open(cashierOptional.get(), true, "Votre caisse est ouvert", shop, cash);
					}else {
						map=response(false, "La caisse est déja ouvert");
					}
				}
			}else {
	...
	public Map<String, Object> findCashierByShop (Long shopId){
